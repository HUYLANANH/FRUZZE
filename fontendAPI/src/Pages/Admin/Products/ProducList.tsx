...
const ProductList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get('/auth/api/Product',{
          headers: {
  ...
  const handleDelete = async (id: string, image: string, listStringImage?: string[]) => {
    ...
    try {
      ...
      await axios.delete(`/auth/api/Product/${id}`);
  
      const imageName = image.split('/').pop();
      console.log('imageName:', imageName);
      if (imageName) {
        await axios.delete(`/auth/api/images?imageName=${encodeURIComponent(imageName)}&isMainImage=true`);
      }
  
      if (listStringImage && listStringImage.length > 0) {
        for (let img of listStringImage) {
          const imageNameInList = img.split('/').pop();
          const deleteImageResponse = await axios.delete(`/auth/api/images?imageName=${encodeURIComponent(imageNameInList!)}&isMainImage=false`);
        }
      }
