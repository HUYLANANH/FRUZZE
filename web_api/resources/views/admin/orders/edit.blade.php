    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const fetchData = async () => {
            try {
                ...
                alert("Có lỗi xảy ra khi tải dữ liệu địa lý. Vui lòng thử lại sau.");
            }
        };

        // Hàm render danh sách thành phố
        const renderCities = (data) => {
            cities.innerHTML = '<option value="" disabled selected>Chọn Thành phố</option>';
            data.forEach((city) => {
            ...
            cities.addEventListener("change", () => renderDistricts(data));
        };

        // Hàm render quận/huyện theo thành phố
        const renderDistricts = (data) => {
            const selectedCityId = cities.options[cities.selectedIndex]?.dataset?.id;
            const cityData = data.find((city) => city.Id === selectedCityId);

            districts.innerHTML = '<option value="" disabled selected>Chọn Quận/Huyện</option>';
            wards.innerHTML = '<option value="" disabled selected>Chọn Phường/Xã</option>';
            ...
            if (cityData) {
                ...
                districts.addEventListener("change", () => renderWards(cityData));
            }
        };

        // Hàm render phường/xã theo quận/huyện
        const renderWards = (cityData) => {
            const selectedDistrictId = districts.options[districts.selectedIndex]?.dataset?.id;
            const districtData = cityData.Districts.find((district) => district.Id === selectedDistrictId);
